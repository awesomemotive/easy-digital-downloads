...
/**
 * Create a simple map of strings used to build checkout form fields.
 *
 * Extensions can filter this list to be able to use
 * `edd_get_checkout_form_field_label()` and `edd_get_checkout_form_field_description()`
 *
 * @see edd_get_checkout_form_field_label()
 * @see edd_get_checkout_form_field_description()
 *
 * @since 3.0
 *
 * @return array
 */
function edd_get_purchase_form_fields() {
	$fields = array(
		'edd_email' => array(
		...
		'edd_first' => array(
			...
			'description' => __( 'We will use this to personalize your account experience.', 'easy-digital-downloads' ),
	...
	/**
	 * Filter the fields available to retrieve label and descriptions for.
	 *
	 * @since 3.0
	 *
	 * @param array $fields Default registered fields.
	 */
	return apply_filters( 'edd_get_purchase_form_fields', $fields );
}

/**
 * Retrieve a label for a registered checkout form field.
 *
 * @since 3.0
 *
 * @param string $key Field key to retrieve.
 * @return mixed False if no field is registered.
 */
function edd_get_purchase_form_field_label( $key ) {
	$fields = edd_get_purchase_form_fields();

	if ( ! isset( $fields[ $key ] ) || ! isset( $fields[ $key ]['label'] ) ) {
		return false;
	}
...
/**
 * Retrieve a description for a registered checkout form field.
 *
 * @since 3.0
 *
 * @param string $key Field key to retrieve.
 * @return mixed False if no field is registered or output is disabled.
 */
function edd_get_purchase_form_field_description( $key ) {
	/**
	 * Filter if the description should appear on the purchase form field.
	 *
	 * @since 3.0.0
	 *
	 * @param bool Output the description or not.
	 * @param string $key Field key to retrieve.
	 */
	if ( ! apply_filters( 'edd_output_purchase_form_field_description', true, $key ) ) {
		return false;
	}

	$fields = edd_get_purchase_form_fields();

	if ( ! isset( $fields[ $key ] ) || ! isset( $fields[ $key ]['description'] ) ) {
		return false;
	}
...
/**
 * Retrieve a placeholder for a registered checkout form field.
 *
 * @since 3.0
 *
 * @param string $key Field key to retrieve.
 * @return mixed False if no field is registered or output is disabled.
 */
function edd_get_purchase_form_field_placeholder( $key ) {
	$fields = edd_get_purchase_form_fields();

	if ( ! isset( $fields[ $key ] ) || ! isset( $fields[ $key ]['placeholder'] ) ) {
		return false;
	}
