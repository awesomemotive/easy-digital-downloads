				...
				<?php } else { ?>
					...
					<form id="edd_purchase_form" action="<?php echo $page_URL; ?>" method="POST">					
						...
						if($can_checkout) { ?>
							...
							<?php } ?>
							<?php if(edd_has_active_discounts()) { // only show if we have at least one discount ?>
...
function edd_get_register_fields() {
	...
	<fieldset id="edd_register_fields">
		...
		<p>
			<input name="edd_email" id="edd_email" class="required edd-input" type="text" placeholder="<?php _e('Email', 'edd'); ?>" title="<?php _e('Email', 'edd'); ?>"/>
