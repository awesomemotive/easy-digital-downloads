...
class Extension_Manager {
	...
	public function do_extension_card( $product, $config, $button_parameters, $link_parameters ) {
		...
		<div class="edd-extension-manager__card">
			<h3 class="edd-extension-manager__title">
				...
				printf( esc_html__( 'Get %s Today!', 'easy-digital-downloads' ), esc_html( $data['title'] ) );
				?>
			</h3>
	...
	public function delete_options( $plugin_basename ) {
		rgc_error_log( 'we would delete these options:' );
		// Delete the option generated by the request to the Products API.
		$type = filter_input( INPUT_POST, 'type', FILTER_SANITIZE_STRING );
		$id   = filter_input( INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT );
		if ( $type && $id ) {
			rgc_error_log( "edd_extension_{$type}_{$id}_data" );
		...
		if ( $item_id ) {
			rgc_error_log( "edd_extension_{$item_id}" );
