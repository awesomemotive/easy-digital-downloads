				...
				<div class="inside">
					...
					<form method="post" enctype="multipart/form-data" action="<?php echo admin_url( 'tools.php?page=edd-settings-export-import' ); ?>">
						...
						<p>
							...
							<?php wp_nonce_field( 'edd_settings_import_nonce', 'edd_settings_import_nonce' ); ?>
...
function edd_process_settings_export() {
	...
	if( empty( $_POST['edd_settings_export_nonce'] ) )
		return;

	if( ! wp_verify_nonce( $_POST['edd_settings_export_nonce'], 'edd_settings_export_nonce' ) )
		return;
...
function edd_process_settings_import() {
	...
	if( empty( $_POST['edd_settings_import_nonce'] ) )
		return;

	if( ! wp_verify_nonce( $_POST['edd_settings_import_nonce'], 'edd_settings_import_nonce' ) )
		return;
	...
	update_option( 'edd_settings_general'   , $settings['general']    );
	update_option( 'edd_settings_gateways'  , $settings['gateways']   );
	update_option( 'edd_settings_emails'    , $settings['emails']     );
	update_option( 'edd_settings_styles'    , $settings['styles']     );
	update_option( 'edd_settings_taxes'     , $settings['taxes']      );
	update_option( 'edd_settings_extensions', $settings['extensions'] );
	update_option( 'edd_settings_misc'      , $settings['misc']       );
