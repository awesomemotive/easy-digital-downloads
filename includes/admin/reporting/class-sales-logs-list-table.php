...
class EDD_Sales_Log_Table extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ){
			...
			case 'customer' :
				$name = ! empty( $item['customer']->name ) ? $item['customer']->name : '#' . $item['customer']->id . ' <em>' . __( 'Unnamed Customer','easy-digital-downloads' ) . '</em>';
				$return = '<a href="' . esc_url( admin_url( 'edit.php?post_type=download&page=edd-customers&view=overview&id=' . $item['customer']->id ) ) . '">' . $name . '</a>';
				break;
