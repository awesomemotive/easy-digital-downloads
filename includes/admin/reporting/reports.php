					...
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							return apply_filters(
								'edd_reports_overview_earnings',
								$stats->get_order_earnings(
									array(
										'range'    => $filter['range'],
										'relative' => true,
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_overview_sales',
								$stats->get_order_count(
									array(
										'range'    => $filter['range'],
										'relative' => true,
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_overview_average_customer_revenue',
								$stats->get_customer_lifetime_value(
									array(
										'function' => 'AVG',
										'range'    => $filter['range'],
										'output'   => 'formatted',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_overview_average_order_value',
								$stats->get_order_earnings(
									array(
										'function' => 'AVG',
										'output'   => 'formatted',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_overview_new_customers',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_overview_busiest_day',
								$stats->get_busiest_day(
									array(
										'range' => $filter['range'],
									)
								)
			...
			array(
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							$d = $stats->get_most_valuable_order_items(
								array(
									'range' => $filter['range'],
								)
							);
			...
			array(
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats  = new EDD\Stats();
							$number = $stats->get_order_refund_count(
								array(
									'range' => $filter['range'],
								)
							);
			...
			array(
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats  = new EDD\Stats();
							$number = $stats->get_order_item_refund_count(
								array(
									'range'  => $filter['range'],
									'status' => array( 'refunded' ),
								)
			...
			array(
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats  = new EDD\Stats();
							$amount = $stats->get_order_refund_amount(
								array(
									'range'  => $filter['range'],
									'output' => 'formatted',
								)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_refunds_average_refund_amount',
								$stats->get_order_refund_amount(
									array(
										'function' => 'AVG',
										'range'    => $filter['range'],
										'output'   => 'formatted',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_refunds_refund_rate',
								$stats->get_refund_rate(
									array(
										'range'  => $filter['range'],
										'output' => 'formatted',
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							return apply_filters(
								'edd_reports_gateways_sales',
								$stats->get_gateway_sales(
									array(
										'range'   => $filter['range'],
										'gateway' => $gateway,
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							return apply_filters(
								'edd_reports_gateways_earnings',
								$stats->get_gateway_earnings(
									array(
										'range'   => $filter['range'],
										'gateway' => $gateway,
										'output'  => 'formatted',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							if ( empty( $gateway ) ) {
								return apply_filters(
									'edd_reports_gateways_average_order_value',
									$stats->get_order_earnings(
										array(
											'range'    => $filter['range'],
											'function' => 'AVG',
					...
					'views' => array(
						'tile' => array(
							'data_callback' => function () use ( $filter, $country, $region ) {
								...
								return $stats->get_tax_by_location(
									array(
										'output'      => 'formatted',
										'range'       => $filter['range'],
										'download_id' => $download['download_id'],
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_file_downloads_number_of_file_downloads',
								$stats->get_file_download_count(
									array(
										'range'       => $filter['range'],
										'download_id' => $download['download_id'],
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_file_downloads_average_per_customer',
								$stats->get_average_file_download_count(
									array(
										'range'  => $filter['range'],
										'column' => 'customer_id',
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_file_downloads_average_per_order',
								$stats->get_average_file_download_count(
									array(
										'range'  => $filter['range'],
										'column' => 'order_id',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_discounts_number_of_discounts_used',
								$stats->get_discount_usage_count(
									array(
										'range' => $filter['range'],
									)
								)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_discounts_ratio_of_discounted_orders',
								$stats->get_ratio_of_discounted_orders(
									array(
										'range' => $filter['range'],
									)
								)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter, $d ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_discounts_customer_savings',
								$stats->get_discount_savings(
									array(
										'range'         => $filter['range'],
										'output'        => 'formatted',
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_discounts_average_discount_amount',
								$stats->get_average_discount_amount(
									array(
										'range'  => $filter['range'],
										'output' => 'formatted',
									)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							...
							$r = apply_filters(
								'edd_reports_discounts_most_popular_discount',
								$stats->get_most_popular_discounts(
									array(
										'range' => $filter['range'],
									)
								)
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () use ( $filter ) {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_customers_average_customer_value',
								$stats->get_customer_lifetime_value(
									array(
										'function' => 'AVG',
										'range'    => $filter['range'],
				...
				'views' => array(
					'tile' => array(
						'data_callback' => function () {
							$stats = new EDD\Stats();
							return apply_filters(
								'edd_reports_customers_average_order_count',
								$stats->get_customer_order_count(
									array(
										'function' => 'AVG',
									)
								)
