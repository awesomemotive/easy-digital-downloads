...
class EDD_Categories_Reports_Table extends WP_List_Table {
	...
	public function reports_data() {
		...
		foreach ( $categories as $category_id => $category ) {
			...
			$paymentStats = new EDD_Payment_Stats();

			foreach ( $downloads as $download ) {
				$currentAverageSales	 = $currentSales			 = $paymentStats->get_sales( $download, $start_date, $end_date );
				$currentAverageEarnings	 = $currentEarnings		 = $paymentStats->get_earnings( $download, $start_date, $end_date );
				...
				if ( $months > 0 ) {
					$currentAverageSales	 = ( $currentSales / $months );
					$currentAverageEarnings	 = ( $currentEarnings / $months );
				}

				$sales += $currentSales;
				$earnings += $currentEarnings;
				$avg_sales += $currentAverageSales;
				$avg_earnings += $currentAverageEarnings;
			}
			...
			$reports_data[] = array(
				...
				foreach ( $child_terms as $child_term ) {
					...
					foreach ( $child_downloads as $child_download ) {
						$currentAverageSales	 = $currentSales			 = $paymentStats->get_sales( $child_download, $start_date, $end_date );
						$currentAverageEarnings	 = $currentEarnings		 = $paymentStats->get_earnings( $child_download, $start_date, $end_date );
						...
						$child_sales += $currentSales;
						$child_earnings += $currentEarnings;
						$child_avg_sales += $currentAverageSales;
						$child_avg_earnings += $currentAverageEarnings;
					}
