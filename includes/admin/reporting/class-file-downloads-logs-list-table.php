		...
		if ( $logs ) {
			foreach ( $logs as $log ) {
				...
				if( ! array_key_exists( $log->download_id, $this->queried_files ) ) {
					$files   = maybe_unserialize( $wpdb->get_var( $wpdb->prepare( "SELECT meta_value from $wpdb->postmeta WHERE post_id = %d and meta_key = 'edd_download_files'", $log->download_id ) ) );
