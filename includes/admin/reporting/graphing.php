...
/**
 * Output a Pie graph using jQuery.flot
 *
 * @since  2.4
 * @param  string $id   Unique ID for the DIV, alphanumeric and '_' only
 * @param  array  $data The Data set in key/value form
...
function edd_pie_graph( $id = '', $data = array(), $args = array() ) {
	...
	<script type="text/javascript">
		var <?php echo $id; ?>_data = [
		<?php foreach ( $data as $label => $value ) : ?>
			<?php echo '{ label: "' . esc_attr( $label ) . '", data: "' . $value . '" },' . "\n"; ?>
		<?php endforeach; ?>
		];

		var <?php echo $id; ?>_options = {
			series: {
				pie: {
					show: true,
					radius: <?php echo $args['radius']; ?>,
				}
			},
			legend: {
				show: <?php echo $args['legend']; ?>,
			},
		};

		<?php if ( true === $args['legend'] && ! empty( $args['legend_formatter'] ) ) : ?>
			<?php echo $id; ?>_options.legend.labelFormatter = <?php echo $args['legend_formatter']; ?>;
		<?php endif; ?>

		jQuery( document ).ready( function($) {
			var <?php echo $id; ?>Chart = $('#edd-pie-graph-<?php echo $id; ?>');
			$.plot( <?php echo $id; ?>Chart, <?php echo $id; ?>_data, <?php echo $id; ?>_options );
		});

	</script>
	<div id="edd-pie-graph-<?php echo $id; ?>" class="edd-pie-graph" style="height: <?php echo $args['height']; ?>px;"></div>
	<?php
	return apply_filters( 'edd_pie_graph_output', ob_get_clean(), $id, $data, $args );
}

/**
 * Allow loading of graphing library jquery.flot
 *
 * @since  2.4
 * @param  array  $required_scripts List of jquery.flot resources to include
 * @return void
 */
function edd_load_graph_scripts( $required_scripts = array() ) {

	$default_scripts = array(
		'jquery.flot',
	);

	$required_scripts = wp_parse_args( $required_scripts, $default_scripts );

	// Use minified libraries if SCRIPT_DEBUG is turned off
	$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';

	foreach ( $required_scripts as $required_script ) {
		$handle   = str_replace( '.', '-', $required_script );
		$url_base = apply_filters( 'edd_graphing_library_url_base', EDD_PLUGIN_URL . 'assets/js/', $required_script );
		wp_enqueue_script( $handle, $url_base . $required_script . $suffix . '.js', array( 'jquery' ), EDD_VERSION );
	}
	?>
	<script type="text/javascript">
		...
		function legendFormatterSales (label, series) {
		...
		function legendFormatterEarnings (label, series) {
		...
		var formatCurrency = function (value) {
			...
			var storeCurrency = '<?php echo edd_get_currency(); ?>';
			var decimalPlaces = <?php echo edd_currency_decimal_filter(); ?>;
