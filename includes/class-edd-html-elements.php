...
class EDD_HTML_Elements {
	...
	public function select( $args = array() ) {
		...
		if ( ! isset( $args['selected'] ) || is_null( $args['selected'] ) || strlen( $args['selected'] ) === 0 || ! $args['selected'] ) {
			$selected = "";
		}
		...
		if ( ! empty( $args['options'] ) ) {
			if ( $args['show_option_none'] ) {
				...
				} elseif ( isset( $args['selected'] ) && ! is_null( $args['selected'] ) && strlen( $args['selected'] ) !== 0 && ! is_array( $args['selected'] ) ) {
