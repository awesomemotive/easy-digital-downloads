...
if ( is_user_logged_in() )
{
		...
		)
	{
		...
		<p class="edd_success">
			...
			<?php _e( 'Your profile has been edited successfully.', 'edd' ); ?>
		</p>
		<?php
	}
	edd_print_errors();
	...
	<form id="edd_profile_editor_form" class="edd_form" action="<?php echo edd_get_current_page_url(); ?>" method="post">
		<fieldset>
			<legend>
				<?php _e( 'Change your Name', 'edd' ); ?>
			</legend>
			<p id="edd_profile_name_wrap">
				<label for="edd_first_name">
			...
			<p id="edd_profile_display_name_wrap">
				...
				<select id="edd_display_name" name="edd_display_name">
					...
					<option <?php selected( $display_name, $current_user->user_nicename ); ?> value="<?php echo $current_user->user_nicename; ?>">
					...
					<?php if ( ! empty( $current_user->first_name ) && ! empty( $current_user->last_name ) ): ?>
						...
						<option <?php selected( $display_name, $current_user->last_name . ' ' . $current_user->first_name ); ?> value="<?php echo $current_user->last_name . ' ' . $current_user->first_name; ?>">
			...
			<legend>
				<?php _e( 'Change your Password', 'edd' ); ?>
			</legend>
			<p id="edd_profile_password_wrap">
				<label for="edd_new_user_pass1">
					<?php _e( 'New Password', 'edd' ); ?>
				</label>
				<input name="edd_new_user_pass1" id="edd_new_user_pass1" class="password edd-input" type="password"/>
				<br />
				<label for="edd_new_user_pass2">
					<?php _e( 'Re-enter Password', 'edd' ); ?>
				</label>
			...
			<p class="edd_password_change_notice">
				<?php _e( 'Please note after changing your password, you must log back in.', 'edd' ); ?>
			</p>
