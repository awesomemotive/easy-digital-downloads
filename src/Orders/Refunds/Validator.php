 ...
 *
 * @since     3.3.0
 * @package   EDD\Orders\Refunds\Validator
 */
...
class Validator {
	 ...
	 *
	 * @param Order        $order Original order being refunded.
	 * @param array|string $order_items If `all` is supplied, then all items eligible for refund are included.
	 * @param array|string $adjustments If `all` is supplied, then all adjustments eligible for refund are included.
	 *
	 * @throws \Exception
	 */
	 ...
	 *
	 * @return array
	 * @throws Invalid_Argument If the order item ID is missing or not in the original order.
	 */
	...
	private function get_all_refundable_order_items() {
		...
		foreach ( $this->order->items as $item ) {
			if ( 'refunded' !== $item->status ) {
				$order_items_to_refund[] = array_merge(
					array(
	 ...
	 *
	 * @return array
	 * @throws Invalid_Argument If the adjustment ID is missing or not in the original order.
	 */
	...
	private function get_all_refundable_adjustments() {
		...
		foreach ( $this->order_adjustments as $adjustment ) {
			if ( 'refunded' !== $adjustment->status ) {
				$adjustments_to_refund[] = array_merge(
					array(
	 ...
	 *
	 * @throws \Exception If the refund amount is 0 or over the maximum allowed.
	 */
	 ...
	 *
	 * @throws \Exception If the refund amount is over the maximum allowed.
	 */
	private function validate_order_item_amounts() {
		foreach ( $this->order->items as $item ) {
			...
			$amount_to_refund = wp_parse_args(
				$this->order_items_to_refund[ $item->id ],
				array(
	...
	private function validate_adjustment_amounts() {
		foreach ( $this->order_adjustments as $adjustment ) {
			...
			$amount_to_refund = wp_parse_args(
				$this->adjustments_to_refund[ $adjustment->id ],
				array(
	...
	private function validate_item_and_add_totals( $original_item, $amounts_to_refund ) {
		...
		foreach ( array( 'subtotal', 'tax', 'total' ) as $column_name ) {
			...
			if ( ! array_key_exists( $column_name, $maximum_refundable_amounts ) ) {
				throw new Exception(
					sprintf(
					/* translators: %s: type of amount being refunded (e.g. "subtotal" or "tax"). Not translatable at this time. */
