...
class Tests_API extends WP_UnitTestCase {
	...
	public function test_get_versions() {
		...
		$this->assertArrayHasKey( 'v2', $this->_api->get_versions() );
	}
	...
	public function test_invalid_auth() {
		...
		// Set a bad token
		$wp_query->query_vars['key']     = $this->_api->get_user_public_key( $this->_user_id );
		$wp_query->query_vars['token']   = hash( 'md5', 'invalid' . $this->_api->get_user_public_key( $this->_user_id ) );
	...
	public function test_invalid_key() {
		...
		$wp_query->query_vars['edd-api'] = 'sales';
		// Set bad auth all together
		$wp_query->query_vars['key']     = current_time( 'timestamp' );
		$wp_query->query_vars['token']   = current_time( 'timestamp' );
	...
	public function test_update_key() {
		...
		// Revoke the key we created in setup
		EDD()->api->revoke_api_key( $this->_user_id );
	...
	public function test_get_customers_auth() {
		global $wp_query;
		$wp_query->query_vars['edd-api'] = 'customers';
		$this->setAuth();
		...
		// v1 was setting the customer 'id' as the 'user_id', incorrectly, this was fixed in v2
		$customer = new EDD_Customer( $out['customers'][0]['info']['id'], true );
		$this->assertEquals( $customer->user_id, $out['customers'][0]['info']['id'] );
	...
	public function test_process_query() {
		...
		$this->assertArrayHasKey( 'id', $out['products'][0]['info'] );
		$this->assertArrayHasKey( 'slug', $out['products'][0]['info'] );
		$this->assertEquals( get_post_field( 'post_name', $download->ID ), $out['products'][0]['info']['slug'] );
		$this->assertArrayHasKey( 'title', $out['products'][0]['info'] );
		$this->assertEquals( $download->get_name(), $out['products'][0]['info']['title'] );
		$this->assertArrayHasKey( 'create_date', $out['products'][0]['info'] );
		$this->assertArrayHasKey( 'modified_date', $out['products'][0]['info'] );
		$this->assertArrayHasKey( 'status', $out['products'][0]['info'] );
		$this->assertEquals( $download->post_status, $out['products'][0]['info']['status'] );
		$this->assertArrayHasKey( 'link', $out['products'][0]['info'] );
		$this->assertArrayHasKey( 'content', $out['products'][0]['info'] );
		$this->assertEquals( $download->post_content, $out['products'][0]['info']['content'] );
		$this->assertArrayHasKey( 'thumbnail', $out['products'][0]['info'] );

		$this->assertArrayHasKey( 'stats', $out['products'][0] );
		$this->assertArrayHasKey( 'total', $out['products'][0]['stats'] );
		$this->assertArrayHasKey( 'sales', $out['products'][0]['stats']['total'] );
		$this->assertEquals( $download->get_sales(), $out['products'][0]['stats']['total']['sales'] );
		$this->assertArrayHasKey( 'earnings', $out['products'][0]['stats']['total'] );
		$this->assertEquals( $download->get_earnings(), $out['products'][0]['stats']['total']['earnings'] );
		$this->assertArrayHasKey( 'monthly_average', $out['products'][0]['stats'] );
		$this->assertArrayHasKey( 'sales', $out['products'][0]['stats']['monthly_average'] );
		$this->assertEquals( $download->get_sales(), $out['products'][0]['stats']['monthly_average']['sales'] );
		$this->assertArrayHasKey( 'earnings', $out['products'][0]['stats']['monthly_average'] );
		$this->assertEquals( $download->get_earnings(), $out['products'][0]['stats']['monthly_average']['earnings'] );
