...
namespace EDD\Vendor\Symfony\Component\Translation\Extractor\Visitor;
...
abstract class AbstractVisitor
{
    ...
    protected function getStringArguments(Node\Expr\CallLike|Node\EDD_Attribute|Node\Expr\New_ $node, int|string $index, bool $indexIsRegex = false): array
    {
        if (\is_string($index)) {
    ...
    protected function hasNodeNamedArguments(Node\Expr\CallLike|Node\EDD_Attribute|Node\Expr\New_ $node): bool
    {
        $args = $node instanceof Node\Expr\CallLike ? $node->getRawArgs() : $node->args;
    ...
    protected function nodeFirstNamedArgumentIndex(Node\Expr\CallLike|Node\EDD_Attribute|Node\Expr\New_ $node): int
    {
        $args = $node instanceof Node\Expr\CallLike ? $node->getRawArgs() : $node->args;
    ...
    private function getStringNamedArguments(Node\Expr\CallLike|Node\EDD_Attribute $node, string $argumentName = null, bool $isArgumentNamePattern = false): array
    {
