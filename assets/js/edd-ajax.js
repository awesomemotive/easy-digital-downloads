...
function eddEventTrigger( el, eventName, data ) {
	var event;
...
eddDocIsReady( function() {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.removed) {
					...
					eddEventTrigger( document.body, 'edd_cart_item_removed', [ response ] );
				}
			}
		...
		$.ajax({
			...
			success: function (response) {
				...
				if( ( store_redirect && item_redirect ) || ( ! store_redirect && item_redirect ) ) {
					...
					eddEventTrigger( document.body, 'edd_cart_item_added', [ response ] );
				}
			}
		}).fail(function (response) {
			if ( window.console && window.console.log ) {
	...
	if( edd_scripts.is_checkout == '1' ) {
		...
		if ( ajax_needed ) {
			...
			eddEventTrigger( document.body, 'edd_gateway_loaded', [ chosen_gateway ] );
		}
	}
	...
	$(document).on('click', '#edd_purchase_form #edd_purchase_submit [type=submit]', function(e) {
		...
		$.post(edd_global_vars.ajaxurl, $('#edd_purchase_form').serialize() + '&action=edd_process_checkout&edd_ajax=true', function(data) {
			...
			if ( $.trim(data) == 'success' ) {
				...
				eddEventTrigger( document.body, 'edd_checkout_error', [ data ] );
			}
		});
		...
		if( 'card_state' != $this.attr('id') && null != state_inputs ) {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if ( is_checkout ) {
						eddEventTrigger( document.body, 'edd_cart_billing_address_updated', [ response ] );
					}
				}
			}).fail(function (data) {
				if ( window.console && window.console.log ) {
...
function edd_load_gateway( payment_mode ) {
	...
	jQuery.post(url, { action: 'edd_load_gateway', edd_payment_mode: payment_mode, nonce: nonce },
		function(response){
			...
			eddEventTrigger( document.body, 'edd_gateway_loaded', [ payment_mode ] );
		}
	);
}
